    ...
    class ProxyHandler:
        ...
        async def rotate(self) -> str:
            while True:
                ...
                self.proxyIndex = self.nextProxy()
        ...
        async with httpx.AsyncClient(headers={"x-csrf-token": accountData["x-csrf-token"]}, cookies={".ROBLOSECURITY": accountData[".ROBLOSECURITY"]}) as client:
            while True:
                ...
                if responseJSON["purchased"]:
                    ...
                    tasks = []
                    embed = DiscordEmbed(title=":3")
                    embed.set_timestamp()
                    embed.set_url(f"https://www.roblox.com/catalog/{limitedId}/yorkavacht")
